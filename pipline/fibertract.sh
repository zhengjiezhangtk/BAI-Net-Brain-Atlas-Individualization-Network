...
if [ $MNI = "False" ] ; then
  ...
  if [ ! -f $subdir/tractseg_output/bundle_segmentations.nii.gz ];then
    ...
    flirt -ref $subdir/tractseg_output/bundle_segmentations_tmp.nii.gz -in $subdir/tractseg_output/bundle_segmentations_tmp.nii.gz -o $subdir/DTI/LowRes_Fibers.nii.gz -applyisoxfm 3 -interp nearestneighbour
