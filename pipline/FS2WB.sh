...
softwaredir=/n04dat01/atlas_group/lma/populationGCN/BAI_Net
...
Subject=${site}
AtlasSpaceFolder="$SUBJECT_DIR"/"$Subject"/"$AtlasSpaceFolder"
FreeSurferFolder=$SUBJECT_DIR/$Subject
HighResMesh="164"
LowResMeshes="32"
T1wFolder=$SUBJECT_DIR/$Subject
InverseAtlasTransform=${SUBJECT_DIR}/${Subject}/acpc_dc2standard
AtlasTransform=${SUBJECT_DIR}/${Subject}/standard2acpc_dc
InflateExtraScale=1
...
if [ ! -d $AtlasSpaceFolder/$NativeFolder ]; then
  mkdir $AtlasSpaceFolder/$NativeFolder
fi
if [ ! -d $T1wFolder/$NativeFolder ]; then
  mkdir $T1wFolder/$NativeFolder
fi
if [ ! -d $AtlasSpaceFolder/fsaverage ]; then
  mkdir $AtlasSpaceFolder/fsaverage
fi
if [ ! -d "$AtlasSpaceFolder"/fsaverage_LR"$LowResMeshes"k ]; then
  mkdir "$AtlasSpaceFolder"/fsaverage_LR"$LowResMeshes"k
fi
if [ ! -d "$T1wFolder"/fsaverage_LR"$LowResMeshes"k ]; then
  mkdir "$T1wFolder"/fsaverage_LR"$LowResMeshes"k
fi
...
#MatrixX=`mri_info "$FreeSurferFolder"/mri/brain.mgz | grep "c_r" | cut -d "=" -f 5 | sed s/" "/""/g`
#MatrixY=`mri_info "$FreeSurferFolder"/mri/brain.mgz | grep "c_a" | cut -d "=" -f 5 | sed s/" "/""/g`
#MatrixZ=`mri_info "$FreeSurferFolder"/mri/brain.mgz | grep "c_s" | cut -d "=" -f 5 | sed s/" "/""/g`
...
#    ${CARET7DIR}/wb_command -surface-apply-warpfield "$T1wFolder"/"$NativeFolder"/"$Subject"."$Hemisphere"."$Surface".native.surf.gii "$InverseAtlasTransform".nii.gz "$AtlasSpaceFolder"/"$NativeFolder"/"$Subject"."$Hemisphere"."$Surface".native.surf.gii -fnirt "$AtlasTransform".nii.gz
    cp "$T1wFolder"/"$NativeFolder"/"$Subject"."$Hemisphere"."$Surface".native.surf.gii "$AtlasSpaceFolder"/"$NativeFolder"/"$Subject"."$Hemisphere"."$Surface".native.surf.gii
  ...
  #Label operations
  for Map in aparc aparc.a2009s BA BN_Atlas ; do
  #for Map in aparc aparc.a2009s BA ; do
    if [ -e "$FreeSurferFolder"/label/"$hemisphere"h."$Map".annot ] ; then
      ...
      ${CARET7DIR}/wb_command  -label-to-border "$AtlasSpaceFolder"/"$NativeFolder"/"$Subject"."$Hemisphere".white.native.surf.gii "$AtlasSpaceFolder"/"$NativeFolder"/"$Subject"."$Hemisphere"."$Map".native.label.gii "$AtlasSpaceFolder"/"$NativeFolder"/"$Subject"."$Hemisphere"."$Map".native.border 
     ${CARET7DIR}/wb_command -set-structure "$AtlasSpaceFolder"/"$NativeFolder"/"$Subject"."$Hemisphere"."$Map".native.border $Structure 
    fi
  done
...
for STRING in "$T1wFolder"/"$NativeFolder"@"$AtlasSpaceFolder"/"$NativeFolder"@native "$AtlasSpaceFolder"/"$NativeFolder"@"$AtlasSpaceFolder"/"$NativeFolder"@native "$AtlasSpaceFolder"@"$AtlasSpaceFolder"@"$HighResMesh"k_fs_LR ${STRINGII} ; do
  ...
  for STRINGII in sulc@dscalar thickness@dscalar curvature@dscalar aparc@dlabel aparc.a2009s@dlabel BA@dlabel BN_Atlas@dlabel ; do
