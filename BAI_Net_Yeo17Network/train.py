...
def train_step(model_parameter, model_dir, model_sample_result, FLAGS):
    ...
    subdir_path2 = [ '{}/{}'.format('/n14dat01/lma/data/HCP_test', str(name).strip()) for name in f.readlines()][:42]
    adj2, features2, y_train, y_val, y_test, train_mask, val_mask, test_mask = load_data(FLAGS.hemisphere, subdirlist=subdir_path2, model_dir=model_dir, MPM=False, uniform=False, MSMAll=True, atlas=FLAGS.atlas)

    # #Load Resesting data
    f = open('/n14dat01/lma/data/HCP_test/sub_list.txt', 'r')
    subdir_path3 = [ '{}/{}'.format('/n14dat01/lma/data/HCP_retest', str(name).strip()) for name in f.readlines()][:42]
    adj3, features3, y_train, y_val, y_test, train_mask, val_mask, test_mask = load_data(FLAGS.hemisphere, subdirlist=subdir_path3, model_dir=model_dir, MPM=False, uniform=False,  MSMAll=True, atlas=FLAGS.atlas)
    ...
    for epoch in range(FLAGS.epochs):
        ...
        if epoch % 5 == 1:
            test_retest_dice = []
            for i in range(len(features2)):
                _, _, output1, _ = evaluate(features2[i],  supports2[i], y_val, val_mask,  placeholders )
                sub = 'test'+subdir_path2[validnum].split('/')[-1]
                savepath = '{}/{}.{}.indi.MSM_weighted.{}.label.gii'.format(model_sample_result, FLAGS.atlas, sub, FLAGS.atlas, FLAGS.hemisphere)
                saveGiiLabel(np.argmax(output1, axis=1), FLAGS.hemisphere, savepath )

                _, _, output2, _ = evaluate(features3[i],  supports3[i], y_val, val_mask,  placeholders )
                sub = 'retest'+subdir_path3[validnum].split('/')[-1]
                savepath = '{}/{}.{}.indi.MSM_weighted.{}.label.gii'.format(model_sample_result, FLAGS.atlas, sub, FLAGS.atlas, FLAGS.hemisphere)
                saveGiiLabel(np.argmax(output2, axis=1), FLAGS.hemisphere, savepath )
...
if __name__ == '__main__':
    ...
    flags.DEFINE_string('atlas', 'Yeo7Network', 'cerebral cortex part')
