...
def train_step(model_parameter, model_dir, model_sample_result, FLAGS):
    ...
    adj2, features2, y_train, y_val, y_test, train_mask, val_mask, test_mask = load_data(FLAGS.hemisphere, subdirlist=subdir_path2, model_dir=model_dir, MPM=False, uniform=True, MSMAll=False, atlas=FLAGS.atlas)

    # #Load Resesting data
    f = open('/n14dat01/lma/data/HCP_test/sub_list.txt', 'r')
    subdir_path3 = [ '{}/{}'.format('/n14dat01/lma/data/HCP_retest', str(name).strip()) for name in f.readlines()][:42]
    adj3, features3, y_train, y_val, y_test, train_mask, val_mask, test_mask = load_data(FLAGS.hemisphere, subdirlist=subdir_path3, model_dir=model_dir, MPM=False, uniform=True,  MSMAll=False, atlas=FLAGS.atlas)
...
if __name__ == '__main__':
    ...
    flags.DEFINE_string('atlas', 'Yeo17Network', 'cerebral cortex part')
