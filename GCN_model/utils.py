...
def load_data(hemisphere, subdirlist=None, MPM=True, modeldir=None, uniform=False, MSMAll=True ):
    """
    ...
    modeldir = str(Path(modeldir).parent)
    #read adj
    adj = load_adj(hemisphere, subdirlist, modeldir, uniform, MSMAll)
    #read label and mask
    y_train, y_val, y_test, train_mask, val_mask, test_mask = load_label_and_mask(hemisphere,  MPM)
    #read feature file
...
def load_label_and_mask(hemisphere,  MPM=False):
    ...
    print('label set', select_label )
    index = np.arange(len(label_arr))
    label_arr = to_categorical(label_arr, 106)
...
def load_adj(hemisphere, subdirlist=None, modeldir=None, uniform=True, MSMAll=True):
    if uniform:
        
        target = '{}/adj_matrix_seed_{}.npz'.format(modeldir, hemisphere)
...
def load_feature(hemisphere, subdirlist=None, MSMAll=True):
    ...
    for subdir in subdirlist:
        subdir = Path(subdir)
        sub = subdir.name
        target = subdir/(sub+'_'+hemisphere+'_probtrackx_omatrix2')/'finger_print_fiber.npz'
...
def meshto32klabel(meshlabel, hemisphere, trans=True):
    ...
    path = '/n04dat01/atlas_group/lma/populationGCN/BAI_Net/Brainnetome/metric_index_{}.txt'.format(hemisphere)
